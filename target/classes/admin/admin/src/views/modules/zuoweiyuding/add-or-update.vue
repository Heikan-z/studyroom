<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <div v-if="type=='cross' && ro.mingcheng" class="seat-select-area">
        <div class="seat-list">
          <span
            v-for="i in parseInt(parent.$storage.getObj('crossObj').number)"
            :key="i"
            :class="[
              'seat-icon',
              selectedSeat === i ? 'selected' : (isSeatOccupied(i) ? 'occupied disabled' : '')
            ]"
            @click="selectSeat(i)"
          >
            {{ i }}号
...
export default {
  ...
  methods: {
    ...
    selectSeat(i) {
      if (this.isSeatOccupied(i) && this.selectedSeat !== i) return;
      this.selectedSeat = i;
      this.ruleForm.zuoweihao = i;
    },
    isSeatOccupied(i) {
      const selected = this.parent.$storage.getObj('crossObj').selected;
      if (!selected) return false;
      return selected.split(',').map(s => s.trim()).includes(i + '');
    }
  }
};
</script>
